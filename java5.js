...
function bee1f () {
    ...
    intervalbee1 = setInterval(() => {
        bee1position += 10; 
        bee1.style.left = bee1position + 'px';
        
        if (bee1position >= window.innerWidth) {
            bee1position = -400; 
...
function wasp1f () {
    ...
    intervalwasp1 = setInterval(() => {
        wasp1position += 10; 
        wasp1.style.left = wasp1position + 'px';
        
        if (wasp1position >= window.innerWidth) {
            wasp1position = -1000; 
...
function bee2f () {
    intervalbee2 = setInterval(() => {
        bee2position -= 10; 
        bee2.style.left = bee2position + 'px';
    
       
        if (bee2position <= -200) {
            bee2position = window.innerWidth; 
        }
    }, 61);  
...
function wasp2f () {
    intervalwasp2 = setInterval(() => {
        wasp2position -= 10; 
        wasp2.style.left = wasp2position + 'px';
    
       
        if (wasp2position <= -600) {
            wasp2position = window.innerWidth; 
        }
    }, 63);  
  ...
  function displayPopUp(score) {
    ...
    // Create buttons for retry and home 
    ...
    homeButton.style.display = (score >= 1) ? "inline-block" : "none"; // Show only if score is 1 or more
...
function june (event) {
    handleElement(event, bee1, -5);
}
function june2 (event) {
    handleElement(event, bee2, -5);
}
  
function july (event) {
    handleElement(event, wasp1, 5);
}
  
function july2 (event) {
    handleElement(event, wasp2, 5);
}
